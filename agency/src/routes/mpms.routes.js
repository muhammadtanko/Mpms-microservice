...
module.exports = (UPLOADS) => {
  ...
  api.get("/:id", async (req, res) => {
    try {
      ...
      let { ok, data, message } = await MpmsController.getSingleAgency(id);
      if (ok) {
  ...
  api.post("/", upload.single("logo"), async (req, res) => {
    try {
      ...
      const { ok, data, message } = await MpmsController.createAgency(body);
      if (ok) {
        res.status(201).json({ ok, data });
      } else {
        res.status(500).json({ ok, message });
  ...
  api.get("/", async (req, res) => {
    try {
      const { ok, data, message } = await MpmsController.getAllAgencies();
      if (ok) {
        res.status(201).json({ ok, data });
      } else {
        res.status(500).json({ ok, message });
